create or replace function cd.ft_cuenta_doc_sel(p_administrador integer, p_id_usuario integer, p_tabla character varying, p_transaccion character varying) returns character varying
    language plpgsql
as
$$
/**************************************************************************
...
**************************************************************************************************************
 ...
 #5	    CD       ETR          17/04/2019      MANUEL GUERRA       Mostrar registros de vi/fa de funcionarios
 #7     CD       ETR          20/08/2019      JUAN                Se agrego AND para la correccion de filtro funcionario al momento de exportar
    ...
    if(p_transaccion='CD_CDOC_SEL')then
        ...
        begin
            ...
            v_consulta:='select
                ...
                cdoc.id_funcionarios, --#8
                (select pxp.list(f.desc_funcionario1) 
                from orga.vfuncionario f
                where f.id_funcionario = any (cdoc.id_funcionarios))::text as funcinarios
...
--                left join cd.vsigema_gral sigra on sigra.tipo_sol_sigema = cdoc.tipo_sol_sigema and sigra.id_sigema = cdoc.id_sigema--
    ...
    ***********************************/
    ...
    elseif(p_transaccion='CD_VIA110REP_SEL')then
        ...
        #FECHA:       27/02/2018
        ***********************************/
    elseif(p_transaccion='CD_PASAFUN_SEL')then
        ...
        begin
            ...
            v_consulta:='select
                        ...
                        from conta.tdoc_compra_venta dcv1
                        where dcv1.id_funcionario = dcv.id_funcionario
                        and dcv1.id_plantilla in ('||v_id_plantilla||','||v_id_plantilla_1||')
                        and dcv1.id_periodo = '||v_parametros.id_periodo||'
                        and dcv1.id_int_comprobante is null
                        and dcv1.id_depto_conta = dcv.id_depto_conta
                        ) as sin_cbte,
                        (select
                        sum(param.f_convertir_moneda(dcv1.id_moneda,'||v_id_moneda_base||',dcv1.importe_excento,dcv1.fecha,''O'',2))
                        from conta.tdoc_compra_venta dcv1
                        where dcv1.id_funcionario = dcv.id_funcionario
                        and dcv1.id_plantilla in ('||v_id_plantilla||','||v_id_plantilla_1||')
                        and dcv1.id_periodo = '||v_parametros.id_periodo||'
        ...
        #FECHA:       27/02/2018
        ***********************************/
    elseif(p_transaccion='CD_PASAFUN_CONT')then
        ...
        begin
            ...
            --Sentencia de la consulta
            v_consulta:='with tcontar as (select
                        count(1) as total
                        from conta.tdoc_compra_venta dcv
                        inner join param.tdepto dep
                        on dep.id_depto = dcv.id_depto_conta
            ...
            v_consulta:=v_consulta||v_parametros.filtro;
            v_consulta:=v_consulta||' group by dcv.id_funcionario,fun.codigo,fun.desc_funcionario2,fun.ci,dcv.id_depto_conta,dep.codigo,dep.nombre,dcv.id_periodo)';
            v_consulta:=v_consulta||' select count(1) from tcontar';
